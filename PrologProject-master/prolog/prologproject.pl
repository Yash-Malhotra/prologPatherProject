...
goto(Input) :- destination(_,Input),
...
goto(Category) :-
    ...
    writeln("Please input a valid start location, or input `options` to see destination categories."),
    read_line_to_string(current_input, Input),
    goto(Input).
%Fail case - No matching destination or category found
goto(_) :- writeln("Please input a valid start location, or input `options` to see destination categories."),read_line_to_string(current_input, Input2), goto(Input2).
...
shortest_path(A,B,Path,Time) :-
	...
	%Because of the nature of finding the shortest weighted path problem on a graph, an increase in distance
	%(In number of edges) exponentially increases runtime, so pathing between two very seperated nodes may take some time.
